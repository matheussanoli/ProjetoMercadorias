...
$(function() {
    ...
    /* Numeric message */
    var numericMsg = "O valor deve ser numerico- 0-9 e separados por pontos!";
    /* minlength message */
        ...
        $(this).find('.required').each(function(i,elm){				
            ...
            if ( $(elm).hasClass('required') && $(elm).hasClass('cep') ){
				...
				if(cepr.resultado == 0){
					$(this).removeClass('valid').addClass('invalid');
					$(this).select();
					$(this).parent().find('span').html('Cep nao encontrado, informe um CEP valido.').fadeOut(500).fadeIn(500).fadeOut(500).fadeIn(500);
					valid = false;
					return false;								
				}else{
					$(this).parent().find('span').fadeOut(500);
